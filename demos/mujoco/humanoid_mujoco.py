...
def init_worker():
    global worker_env
    worker_env = gym.make('Humanoid-v5')

# Define the evaluation function for a genome
def evaluate_genome(genome, env=None, render=False, max_steps=1500):
    # Determine which environment to use
    if env is not None:
        # Use the provided environment (for serial evaluation or rendering)
        use_env = env
    else:
        # Use the worker environment (for parallel evaluation)
        use_env = worker_env
    ...
    try:
        observation_data = use_env.reset()
    except (pygame.error, AttributeError):
        # Environment was closed or invalid, create a new one
        if render:
            use_env = gym.make('Humanoid-v5', render_mode='human')
        else:
            use_env = gym.make('Humanoid-v5')
        observation_data = use_env.reset()
    ...
    while True:
        ...
        # Scale outputs to [-0.4, 0.4] range (Humanoid action space)
        action = np.clip(outputs, -0.4, 0.4)
        
        # Handle both old (4 return values) and new (5 return values) Gym API
        step_result = use_env.step(action)
        ...
        if render:
            try:
                use_env.render()
                if hasattr(use_env.unwrapped, 'mujoco_renderer'):
                    use_env.unwrapped.mujoco_renderer.viewer._hide_overlay = True
                    use_env.unwrapped.mujoco_renderer.viewer._hide_menu = True
            except (pygame.error, AttributeError):
                # Display was closed or renderer not available, skip rendering
                pass
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Humanoid NEAT')
    ...
    # 348 inputs (observations) + 1 bias = 349 inputs, 17 outputs (actions)
    init_struct = pnt.GenomeInitStruct()
    init_struct.NumInputs = 349
    init_struct.NumOutputs = 17
    ...
    # Create process pool once if using parallel
    pool = None
    if not args.serial:
        ...
        for gen in tqdm(range(1, generations), desc="Generations"):
            ...
            if args.serial:
                ...
                for species in pop.m_Species:
                    for individual in species.m_Individuals:
                        fitness = evaluate_genome(individual, env=temp_env)
                ...
                # Evaluate genomes in parallel using the persistent pool
                fitnesses = pool.starmap(evaluate_genome, [(g,) for g in genomes])
            ...
            if best_genome and gen % 100 == 0:
                ...
                for i in range(10):
                    ...
                    env_render = gym.make('Humanoid-v5', render_mode='human')
                    try:
                        evaluate_genome(best_genome, env=env_render, render=True, max_steps=1500)
                    finally:
                        env_render.close()
